...
mod tests {
    ...
    impl IEip712 for Eip712 {
        const NAME: &'static str = "Erc20PermitExample";
    ...
    fn create_permit_signature(
        ...
        let domain_separator =
            contract.sender(signer_account.address()).domain_separator();

        // Create the final hash to sign
        let message_hash =
            eip712::to_typed_data_hash(&domain_separator, &struct_hash);

        let signature = signer_account
            .signer()
    ...
    fn permit(
        ...
        let allowance =
            contract.sender(owner).allowance(owner.address(), spender);
        assert_eq!(
            allowance, value,
    ...
    // 1 hour ago     let initial_nonce =
    // contract.sender(owner).nonces(owner.address());
    ...
    //         matches!(
    //             result.unwrap_err(),
    //             Error::ExpiredSignature(ERC2612ExpiredSignature { deadline:
    // expired_deadline })             if expired_deadline == deadline
    //         ),
    ...
    //     let new_nonce = contract.sender(owner).nonces(owner.address());
    //     assert_eq!(new_nonce, initial_nonce, "Nonce should not be
    // incremented"); }
    ...
    //     let value = uint!(100_U256);
    //     let deadline = U256::from(block::timestamp()) + uint!(3600_U256); //
    // 1 hour from now     let initial_nonce =
    // contract.sender(owner).nonces(owner.address());
    ...
    //         matches!(
    //             result.unwrap_err(),
    //             Error::InvalidSigner(ERC2612InvalidSigner { signer, owner:
    // owner_addr })             if owner_addr == owner.address() && signer
    // != owner.address()         ),
    ...
    //     spender: Account,
    // ) {
    //     // This test simulates a domain separator change (like in a chain
    // fork)     // In a real test, we would need to modify the chain ID or
    // other domain     // components For now, we'll just demonstrate the
    // concept
    ...
    //     // In a real test, we would verify that the same signature fails
    // after a     // domain separator change
    // }
}
